...
class StarsHome(DataMixin, ListView):
    ...
    def get_queryset(self):
        return models.Star.objects.filter(is_published=True).select_related('cat')


def about(request):
    return render(request, 'stars/about.html', {'menu': menu, 'about': "О сайте"})
...
class StarsCategory(DataMixin, ListView):
    ...
    def get_queryset(self):
        return models.Star.objects.filter(cat__slug=self.kwargs['cat_slug'], is_published=True).select_related('cat')
...
def type(request, name_type):
    return HttpResponse("Страница приложения Звезды")
